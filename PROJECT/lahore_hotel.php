...
<body>
  ...
  <!--  <h2>Peshawar</h2> -->
    ...
    <form action="lahore_hotel.php" method='POST'>
    
    <p style="font-size:32px;padding-left:50px">Grand Palm Hotel</p>
    <div style='margin-top:-60px;padding-left:200px' >
   <button type='submit' name='HOTEL-1' >BOOK</button>
   <p>Rs 10000</p>
</div>
   <p style="font-size:32px;padding-left:50px;">Luxus Grand Hotel</p>
   <div style='margin-top:-60px;padding-left:200px' >
   ...
   <?php
       ...
       $query = $conn->prepare('UPDATE users set hotel_id="GPH301" where User_id=?');
       $query->execute([$Id]);
    ...
    if (isset($_POST['HOTEL-2']) ){
        $query = $conn->prepare('UPDATE users set hotel_id="LGH401" where User_id=?');
        $query->execute([$Id]);
