...
<body>
  ...
  <!--  <h2>Peshawar</h2> -->
    ...
    <form action="islamabad_hotel.php" method='POST'>
    
    <p style="font-size:32px;padding-left:50px">Hotel Grand Area</p>
    <div style='margin-top:-60px;padding-left:200px' >
   <button type='submit' name='HOTEL-1' >BOOK</button>
   <p>Rs 15000</p>
</div>
   <p style="font-size:32px;padding-left:50px;">Oriole Guest House</p>
   <div style='margin-top:-60px;padding-left:200px' >
   ...
   <?php
       ...
       $query = $conn->prepare('UPDATE users set hotel_id="HGR101" where User_id=?');
       $query->execute([$Id]);
    ...
    if (isset($_POST['HOTEL-2']) ){
        $query = $conn->prepare('UPDATE users set hotel_id="OGR201" where User_id=?');
        $query->execute([$Id]);
